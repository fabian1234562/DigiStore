#!/bin/bash
# Generate real product images using z-ai image
# Saves progress after each image so we can resume
OUTDIR="/home/z/my-project/public/products"
PROGRESS="/home/z/my-project/scripts/gen-progress.txt"
LOG="/home/z/my-project/scripts/gen-log.txt"

mkdir -p "$OUTDIR"
echo "" > "$LOG"
echo "Started $(date)" >> "$LOG"

# Read last completed platform to allow resuming
LAST_DONE=""
if [ -f "$PROGRESS" ]; then
  LAST_DONE=$(tail -1 "$PROGRESS")
  echo "Resuming from: $LAST_DONE" >> "$LOG"
fi
...
gen() {
  ...
  # Resume support
  if [ "$SKIP" -eq 0 ] && [ -n "$LAST_DONE" ]; then
    if [ "$platform" != "$LAST_DONE" ]; then
      return 0
    fi
    SKIP=1
    return 0
  ...
  local slug=$(echo "$platform" | tr '[:upper:]' '[:lower:]' | sed 's/+//g; s/[^a-z0-9]/-/g; s/--*/-/g; s/^-*//-; s/-*$//')
  local outfile="$OUTDIR/${slug}.png"
  
  # Skip if file exists and is larger than 10KB
  if [ -f "$outfile" ] && [ $(stat -c%s "$outfile" 2>/dev/null || echo 0) -gt 10000 ]; then
    echo "[SKIP] $platform (exists)" >> "$LOG"
    echo "$platform" >> "$PROGRESS"
    return 0
  fi
  
  echo -n "[GEN] $platform... " >> "$LOG"
...
echo "Images generated: $(cat $PROGRESS | wc -l)" >> "$LOG"
